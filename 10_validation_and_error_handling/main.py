...
email: str = Field(pattern=r"^[^@\s]+@[^@\s]+.[^@\s]+$", examples=["foo@example.com
"])
password: str = Field(min_length=8, examples=["verysecret!"])
...
- Field制約（min_length / pattern）
- 独自validator（password）
"""
# DBがないので仮のIDを返す
...
return {"user_id": order.user_id, "items": len(order.items), "total_qty": total_qty}

@app.get("/errors/demo")
def error_demo(kind: Annotated[str, Query(pattern="^(http|crash)$")] = "http"):
"""
HTTPException（想定エラー）と、想定外例外（クラッシュ）を比較する。
"""
if kind == "http":
raise HTTPException(status_code=404, detail="this is an intentional HTTP error")
# 想定外例外（500）をわざと起こす
1 / 0
